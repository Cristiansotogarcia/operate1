          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                ...
                className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-violet-500 disabled:bg-gray-50" />
            </div>
            <div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b">
                    ...
                    {ACTIONS.map(a => (
                      <th key={a} className="text-center px-3 py-2 text-xs font-semibold text-gray-500 uppercase cursor-pointer hover:text-violet-600"
                        onClick={() => toggleAll(a)}>
                        {a.replace('can_', '')}
                      </th>
                    ))}
                        ...
                        <td key={a} className="text-center px-3 py-2">
                          <button type="button" onClick={() => togglePerm(m.key, a)}
                            className={`w-6 h-6 rounded flex items-center justify-center transition-colors ${
                              permMap[m.key][a] ? 'bg-violet-600 text-white' : 'bg-gray-100 text-gray-300 hover:bg-gray-200'
                            }`}>
